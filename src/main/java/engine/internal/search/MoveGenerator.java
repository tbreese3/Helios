...
public interface MoveGenerator {
   ...
   *
   * <ul>
   *   <li>{@code ALL} all pseudo-legal moves
   *   <li>{@code CAPTURES} only moves that capture an enemy piece (plus queen promotions,
   *       Stockfish-style)
   *   <li>{@code QUIETS}  all non-capture moves, including under-promotions
   *   <li>{@code EVASIONS} moves that get the side-to-move out of check (captures + blocks + king
   *       moves + castling if legal)
   * </ul>
   */
  enum GenMode {
    ALL,
  ...
  /**
   * Fills {@code buf} with moves for the position encoded in {@code packedPosition}.
   *
   * @param packedPosition the 13-long[] position in the layout defined by {@link
   *     PackedPositionFactory}
   * @param buf caller-owned buffer to receive packed moves (must be large enough for MAX_MOVES ≈
   *     218)

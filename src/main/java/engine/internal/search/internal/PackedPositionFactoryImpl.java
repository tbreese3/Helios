...
public final class PackedPositionFactoryImpl implements engine.internal.search.PackedPositionFactory {
    ...
    @Override public Position fromBitboards(long[] bitboards) {
    ...
    @Override public void makeMoveInPlace(long[] bb, int mv) {
        ...
        if (type == 3) {
            switch (to) {
                case 62 -> { bb[WR] ^= 1L << 63; bb[WR] |= 1L << 61; } // white O-O
                case 58 -> { bb[WR] ^= 1L << 56; bb[WR] |= 1L << 59; } // white O-O-O
                case  6 -> { bb[BR] ^= 1L <<  7; bb[BR] |= 1L <<  5; } // black O-O
                case  2 -> { bb[BR] ^= 1L <<  0; bb[BR] |= 1L <<  3; } // black O-O-O
            }
        }
        ...
        if (pawnMove && Math.abs(to - from) == 16) {
            ...
            meta = (meta & ~EP_MASK) | (63L << EP_SHIFT);          // none
    ...
    private static long updateCastling(long cr, int mover, int from, int to) {
        switch (mover) {
            ...
            case WR -> { if (from ==  7) cr &= ~0b0001; else if (from == 0) cr &= ~0b0010; }
            case BR -> { if (from == 63) cr &= ~0b0100; else if (from ==56) cr &= ~0b1000; }
        }
        // rook captured?
        if (to == 7)  cr &= ~0b0001;
        if (to == 0)  cr &= ~0b0010;
    ...
    public static long popLsb(long b)      { return b & (b - 1); }
    ...
    private static final class PackedPosition implements Position {
        ...
        @Override public String toFen()             { return toFenString(); }
        ...
        private char pieceCharAt(int sq) {
            for (int i = 0; i < 12; ++i) if ((bb[i] & (1L<<sq)) != 0)

...
import engine.internal.search.internal.PackedPositionFactoryImpl;
...
public interface PackedPositionFactory {
  ...
  record Diff(int move, int from, int to, int mover, int capturedIdx, long oldMeta) {}

  /* fast helpers – **signatures only** */
  boolean makeMoveInPlace(
      long[] bb, byte[] board, int mv, MoveGenerator gen, PackedPositionFactoryImpl.FastDiff diff);

  void undoMoveInPlace(long[] bb, byte[] board, PackedPositionFactoryImpl.FastDiff d);

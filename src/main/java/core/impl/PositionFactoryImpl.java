...
public final class PositionFactoryImpl implements PositionFactory {
  ...
  */
  ...
  // Remove EP key if one was present in the "before" state.
  // chesslib does this unconditionally, without a legality check.
    int oldEpDest = (metaOld & EP_BITS) >>> EP_SHIFT;
    if (oldEpDest != EP_NONE) {
      // The side TO MOVE in the old state is the one that could capture.
      // The pawn that can BE captured is on the rank behind the destination square.
      boolean stmIsWhiteInOldMeta = (metaOld & STM_BIT) == 0;
      int targetPawnSquare = stmIsWhiteInOldMeta ? oldEpDest - 8 : oldEpDest + 8;
      h ^= EP_SQ_KEY[targetPawnSquare];
    }
